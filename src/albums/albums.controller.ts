...
export const createAlbum: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        console.error('[albums.controller][createAlbums][Error]');
        res.status(500).json({
    ...
    try {
        ...
        req.body.tracks.forEach(async (track: Track, index: number) => {
            try{
                ...
                console.error('[albums.controller][updateAlbum][Error]');
                res.status(500).json({
        ...
        console.error('[albums.controller][updateAlbums][Error]');
        res.status(500).json({
...
async function readTracks(albums: Album[], res: Response<any, Record<string, any>>) {
    for (let i = 0; i < albums.length; i++) {
        try {
            ...
            console.error('[albums.controller][readTracks][Error]');
            res.status(500).json({
...
export const deleteAlbum: RequestHandler = async (req: Request, res: Response) => {
    try {
        ...
        console.error('[albums.controller][deleteAlbums][Error]');
        res.status(500).json({
            message: 'There was an error when deleting the albums'
        });
    }
};
